...
| 验收 | 回读仅认「已暂停/暂停中」；拒绝裸「暂停」、可暂停、toast/开关单独信号 |
| 仍关闭 | 放量、批量启停、出价、改排期 |
...
| LEGACY-4 | ?? | `/health.secret_files` 增加默认第三方回调告警；仍需生产自设 `DIAN_AGENT_OAUTH_CALLBACK_URL` |
...
- 暂停成功只认本行「已暂停/暂停中」；等待期持续处理延迟确认弹窗；不认 toast / 开关 off / 裸「暂停」
- 成功判定排除按钮/开关文案，且须离开授权中的投放态（防 loading「暂停中」假成功）
- 确认后弹窗残留 DOM 时仍以行状态为准，避免假失败
- Bridge `pause_plan_succeeded` 与 content 对齐；裸「暂停」不得 verify；「可暂停」不得当 inactive
